...
class TrackingConfig:
    ...
    gating_threshold: float = 30.0 # More forgiving gating for fast movements
    confidence_threshold: float = 0.35 # Reverted to standard stability value
    lost_reinit_threshold: int = 45    # 1.5 second delay before full reinit
    measurement_blend_min: float = 0.20
    measurement_blend_max: float = 0.85 # Trust measurement more
    ...
    velocity_lookahead_factor: float = 0.0
    ...
    mediapipe_correction_interval_frames: int = 3 # ULTRA HIGH FREQUENCY Ground Truth
    ...
    one_euro_min_cutoff: float = 1.0 # Snappier baseline (reduced low-speed lag)
    one_euro_beta: float = 0.05      # High-speed responsiveness (reduced turn lag)
    trajectory_history_length: int = 15 # Number of frames for the meteor trail
